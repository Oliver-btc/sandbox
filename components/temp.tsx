...
export function QRCodeGeneratorPage() {
  ...
  const calculateTrustAndLoyaltyScore = (baseRewards: Reward[], totalQRCodes: number): number => {
    ...
    const score = (baseRewardFrequency * totalBaseRewardValue) / totalQRCodes;
    return Math.min(Math.max(score, 0), 100); // Ensure the score is between 0 and 100
  };
  
  const calculateExcitementScore = (midRewards: Reward[], totalQRCodes: number): number => {
    const totalMidRewardValue = midRewards.reduce((sum, reward) => sum + (reward.amount * reward.count), 0);
    const score = (totalMidRewardValue / totalQRCodes) * 10; // Multiplying by 10 to scale it up
    return Math.min(Math.max(score, 0), 100); // Ensure the score is between 0 and 100
  };
  
  const calculateHypeScore = (topRewards: Reward[], productPrice: number, totalQRCodes: number): number => {
